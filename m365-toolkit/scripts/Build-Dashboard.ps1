...
$repositoryUrl = "https://github.com/Thugney/tenantscope"
try {
    $gitRepoUrl = (& git -C (Split-Path $scriptRoot -Parent) config --get remote.origin.url 2>$null)
    if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($gitRepoUrl)) {
        $normalizedRepoUrl = $gitRepoUrl.Trim()
        if ($normalizedRepoUrl -match '^git@github\.com:(.+?)(?:\.git)?$') {
            $repositoryUrl = "https://github.com/$($Matches[1])"
        }
        else {
            $repositoryUrl = $normalizedRepoUrl -replace '\.git$',''
        }
    }
}
catch {
    # Fall back to the canonical public repo URL when git metadata is unavailable.
}
...
$bundleLines += "window.__M365_REPOSITORY_URL = `"$repositoryUrl`";"
